...
with tab_LSM :
    ...
    with tab1 :
        ...
        activer_pricing_LSM = st.button('Pricing LSM')
        
        col11, col12, col13 = st.columns(3)
        
        with col11 : 
            date_pricing_LSM = st.date_input("Entrez une date de pricing LSM:", value=today)
            
        with col13 : 
            nb_pas_LSM = st.number_input("Entrez le nombre de pas utilisé pour le pricing LSM:", 1, 5000, value=100, step=1)
        ...
        dividende_check_LSM = st.toggle("Dividende LSM", value=False)

        col21, col22, col23 = st.columns(3)

        with col21 : 

            spot_LSM = st.number_input("Entrez le prix de départ du sous-jacent (€):", format="%.2f",value=100.0, step=0.01)
            
        with col22:
            
            volatite_LSM = st.number_input("Entrez le niveau de volatilité (%):", format="%.2f", value=20.0, step=1.00)/100
            
        with col23:
            risk_free_rate_LSM = st.number_input("Entrez le niveau de taux d'intérêt (%):", format="%.2f", value=4.0, step=1.00)/100
        
        if dividende_check_LSM : 
            with col21 : 
                dividende_ex_date_LSM = st.date_input("Entrez la date de dividende:")
            with col22:
                dividende_montant_LSM = st.number_input("Entrez le montant du dividende (%):", format="%.2f" ,value=0.0, step=1.00)
        else : 
            dividende_ex_date_LSM = today
            dividende_montant_LSM = 0
        ...
        with col31:
            maturite_LSM = st.date_input("Entrez une date de maturité LSM:",value=today+ timedelta(days=10))
            
        col41, col42, col43 = st.columns(3)
            
        with col41 :
            strike_LSM = st.number_input("Entrez le strike (€):", format="%.2f",value=100.0, step=0.01)
        
        with col42:
            option_type_LSM = st.selectbox("Choisissez le type de l'option LSM:", ['Call', 'Put'])
            
        with col43:
            option_exercice_LSM = st.selectbox("Choisissez le type de l'exercice LSM:", ['Européenne','Américaine']) 

        
        #Ici, on feed les objets
                    
        donnee_marche = DonneeMarche(date_debut=date_pricing_LSM, prix_spot=spot_LSM, volatilite=volatite_LSM, 
                                     taux_interet = risk_free_rate_LSM, taux_actualisation=risk_free_rate_LSM, 
                                     dividende_ex_date=dividende_ex_date_LSM, dividende_rate=dividende_montant_LSM)
        option = Option(maturite=maturite_LSM, prix_exercice=strike_LSM, 
                        call=True if option_type_LSM == "Call" else False,
                        americaine=False if option_exercice_LSM == 'Européenne' else True,
                        date_pricing=date_pricing_LSM)

        bs_check = not option.americaine and donnee_marche.dividende_montant == 0 


        if activer_pricing_LSM : 
            if  bs_check : 
                bns = BlackAndScholes(modele=option)
